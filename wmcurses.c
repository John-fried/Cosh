...
void win_handle_mouse(void)
{
	...
	if (wm.drag_win) {
		if (ev.bstate & (BUTTON1_RELEASED | BUTTON1_CLICKED)) {
			...
			wm.drag_win->y =
			    ev.y - wm.drag_win->drag_state.drag_off_y;
			wm.drag_win->x =
			    ev.x - wm.drag_win->drag_state.drag_off_x;
			...
			move_panel(wm.drag_win->panel, wm.drag_win->y,
				   wm.drag_win->x);
			win_needs_redraw = 1;
		}
	...
	for (int i = wm.count - 1; i >= 0; i--) {
		...
		if (ev.y >= w->y && ev.y < (w->y + w->h) &&
			...
			if (w->last_seq != WIN_SEQ_NONE && w->input_cb) {
				w->input_cb(w, (int)w->last_seq, &ev);
				w->dirty = 1;
			}
